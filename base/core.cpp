...
//
const char* g_logFolder = "log_folder";

//#include <axion/clientlib.hpp>
//#include <virtualizer/VirtualizerSDK.h>
//
//#ifdef _WIN64
//#pragma comment(lib, "virtualizer/VirtualizerSDK64.lib")
//#pragma comment(lib, "axion/clientlib64.lib")
//#else
//#pragma comment(lib, "virtualizer/VirtualizerSDK32.lib")
//#pragma comment(lib, "axion/clientlib32.lib")
//#endif

DWORD WINAPI OnDllAttach(uintptr_t lpParameter)
{
	try
	{
		...
		// will log to %appdata%/log_folder/log.txt
		//if (!Auth::Init(g_logFolder, (uintptr_t)lpParameter)) {
	...
	catch (const std::exception& ex)
	{
		...
		#else
		// unload
		FreeLibraryAndExitThread(reinterpret_cast<HMODULE>(lpParameter), EXIT_FAILURE);
		#endif
	}
	//VIRTUALIZER_END
	return 1UL;
...
DWORD WINAPI OnDllDetach(LPVOID lpParameter)
{
	...
	FreeLibraryAndExitThread(reinterpret_cast<HMODULE>(lpParameter), EXIT_SUCCESS);
}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		// create main thread
		if (auto hThread = CreateThread(nullptr, 0U, (LPTHREAD_START_ROUTINE)OnDllAttach, hModule, 0UL, nullptr); hThread != nullptr)
			CloseHandle(hThread);

		// create detach thread
		if (auto hThread = CreateThread(nullptr, 0U, (LPTHREAD_START_ROUTINE)OnDllDetach, hModule, 0UL, nullptr); hThread != nullptr)
			CloseHandle(hThread);
