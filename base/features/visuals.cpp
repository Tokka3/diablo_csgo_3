...
void CVisuals::Player(CBaseEntity* pLocal, CBaseEntity* pEntity, Context_t& ctx, const float flDistance, const Color& colInfo, const Color& colFrame, const Color& colOutline)
{
	...
	if (!I::Engine->GetPlayerInfo(pEntity->GetIndex(), &playerInfo))
		...
		//for (int i = 1; i < 127; i++)
		//{
		//	auto pBone = pEntity->GetBonePosition(i - 1);
		//	auto pParentBone = pEntity->GetBonePosition(i);
		//	// auto pParentBone = pStudioHdr->GetBone(pBone->iParent);
		//	ImVec2 childPos;
		//	 ImVec2 ParentPos;
		//	D::WorldToScreen(pBone.value(), childPos);
		//	 D::WorldToScreen(pParentBone.value(), ParentPos);
		//	//const auto pParentBone = pStudioHdr->GetBone(pBone->iParent);
		//	/*ImVec2 pBonePos(pBone->vecPosition.x, pBone->vecPosition.y);
		//	ImVec2 pParentBonePos(pBone->vecPosition.x, pParentBone->vecPosition.y);
			...
			if (const auto pModel = pEntity->GetModel(); pModel != nullptr)
			{
				...
				if (const auto pStudioHdr = I::ModelInfo->GetStudioModel(pModel); pStudioHdr != nullptr)
				{
					for (int i = 0; i < 127; i++)
					{
						...
						if (const auto pBone = pStudioHdr->GetBone(i); pBone != nullptr && pBone->iFlags & BONE_USED_BY_HITBOX) {
							if (const auto parentBone = pStudioHdr->GetBone(pBone->iParent); parentBone != nullptr && parentBone->iFlags & BONE_USED_BY_HITBOX) {

								ImVec2 childPos;
								ImVec2 ParentPos;
								D::WorldToScreen(pEntity->GetBonePosition(i).value(), childPos);
...
void CVisuals::Box(const Box_t& box, const int nBoxType, const Color& colPrimary, const Color& colOutline)
{
	...
	case EVisualsBoxType::FULL:
	{
		...
		if (outline) {
			D::AddRect(ImVec2(box.left, box.top), ImVec2(box.right, box.bottom), outlineColor, DRAW_RECT_BORDER | DRAW_RECT_OUTLINE, outlineColor);
		}
		D::AddRect(ImVec2(box.left, box.top), ImVec2(box.right, box.bottom), espColor);
